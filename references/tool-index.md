...
由实际 2.5.1 注册表生成。默认21工具；诊断 profile 仅3工具，legacy 为30工具且仅用于独立兼容回归。
...
| `pcb_execute_plan` | 执行/规则 | planPath，plan，mode=validate/prepare/execute，guard；布局轮次最多100个展开操作；计划含原点锚定板框、孔槽、焊盘、过孔、器件和铜；写入前后阻止跨对象焊盘重叠，返回 padOverlapGate/boardDelta/workflowReceipt，错误附 recoveryDirective |

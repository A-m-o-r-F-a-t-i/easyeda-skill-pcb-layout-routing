...
由实际 2.6.0 注册表生成。默认21工具；诊断 profile 仅3工具，legacy 为30工具且仅用于独立兼容回归。
...
几何 mode=build 从原生对象补旧值与受影响网络；mode=reconcile 两次稳定只读并返回已应用/待执行/冲突及 remainingPlan。mode=validate不连接编辑器；mode=prepare绑定状态并运行整批候选预检，阻断时不发写入；mode=execute复用同状态预检或重新检查，再逐项独立读回。铜距/孔距使用计划下限，未知覆盖不当作通过。`pcb_cleanup_components`使用自己的 preflight/execute 两阶段 guard。新圆形板框以 `[0,0]` 为圆心，多边形含 `[0,0]` 顶点；布局/重布局省略批次时，不超过100个展开操作作为一个完整轮次。执行期阻止不同组件焊盘重叠及独立焊盘/过孔侵入元件焊盘，翻面或换层后的真实重叠会回滚元件，并返回 `padOverlapGate`；未 `CLEAR/NOT_APPLICABLE` 不得进入下一轮。结果统一返回 `workflowReceipt`，执行结果同时返回 `boardDelta`，只有 created/modified/deleted 计为真实板上变化。部分或未知结果附 `recoveryDirective`，调用方只读取 `minimumReadScope`、禁止重放并只续作剩余后缀，随后返回已保存的父 PCB 阶段。
...
| `pcb_execute_plan` | 执行/规则 | planPath，plan，mode=build/reconcile/validate/prepare/execute，guard；布局轮次最多100个展开操作；计划含原点锚定板框、孔槽、焊盘、过孔、器件和铜；写入前后阻止跨对象焊盘重叠，返回 padOverlapGate/boardDelta/workflowReceipt，错误附 recoveryDirective |
...
| `pcb_audit_geometry` | 验证 | snapshot，toleranceMil，detailLimit，checks=geometry/connectivity/groupQuality，groups，referenceLayers，net，nativeUnroutedCount |
...
- audit的checks选择geometry、connectivity或groupQuality；后者按显式功能组统计距离、Line/Arc 总长、过孔、区域出口和参考层占用，不能把缺失网络记成零，也不验证地铜连续或整板通过。

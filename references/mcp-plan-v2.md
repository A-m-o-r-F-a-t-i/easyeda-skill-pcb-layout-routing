...
`easyeda-pcb-mcp`是PCB专用执行层。它负责读取当前PCB、校验并执行显式板框、独立孔槽/焊盘、铜/文字计划、重建覆铜、管理约束、受保护导入原理图变更、逐项读回、保存和批量DRC。它不负责电路设计、自动布局、自动寻路或自动布线。完整2.3.0生产工具路由见[工具索引](tool-index.md)。
